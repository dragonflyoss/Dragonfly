...
func NewManager(cfg *config.Config, progressManager mgr.ProgressMgr,
	...
	cacheStore, err := sm.Get(store.LocalStorageDriver)
	if err != nil {
		return nil, err
	}
	...
	return &Manager{
		...
		limiter:         ratelimiter.NewRateLimiter(ratelimiter.TransRate(int64(cfg.MaxBandwidth-cfg.SystemReservedBandwidth)), 2),
