...
func init() {
	check.Suite(&DFGetP2PDynamicTestSuite{})
}

type DFGetP2PDynamicTestSuite struct {
	starter *command.Starter
}

func (s *DFGetP2PDynamicTestSuite) SetUpSuite(c *check.C) {
	s.starter = command.NewStarter("DFGetP2PDynamicTestSuite")
	if _, err := s.starter.Supernode(0); err != nil {
		panic(fmt.Sprintf("start supernode failed:%v", err))
	}
}

func (s *DFGetP2PDynamicTestSuite) TearDownSuite(c *check.C) {
	s.starter.Clean()
}

func (s *DFGetP2PDynamicTestSuite) TestDownloadFileDynamicMode(c *check.C) {
	var cases = []struct {
		filePath    string
		fileContent []byte
		targetPath  string
