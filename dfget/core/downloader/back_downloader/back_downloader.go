...
type BackDownloader struct {
	...
	// cfg holds all configs of runtime.
	cfg *config.Config
...
func (bd *BackDownloader) Run() error {
	...
	logrus.Infof("start download %s from the source station", path.Base(bd.cfg.RV.RealTarget))

	defer bd.Cleanup()

	prefix := "backsource." + bd.cfg.Sign + "."
	if f, err = ioutil.TempFile(path.Dir(bd.cfg.RV.RealTarget), prefix); err != nil {
		return err
	}
	bd.tempFileName = f.Name()
	defer f.Close()

	if resp, err = cutil.HTTPGetWithHeaders(bd.cfg.URL, cutil.ConvertHeaders(bd.cfg.Header)); err != nil {
		return err
	}
	...
	reader := cutil.NewLimitReader(resp.Body, bd.cfg.LocalLimit, bd.cfg.Md5 != "")
	if _, err = io.CopyBuffer(f, reader, buf); err != nil {
		return err
	}

	realMd5 := reader.Md5()
	if bd.cfg.Md5 == "" || bd.cfg.Md5 == realMd5 {
		err = downloader.MoveFile(bd.tempFileName, bd.cfg.RV.RealTarget, "")
	} else {
		err = fmt.Errorf("md5 not match, expected:%s real:%s", bd.cfg.Md5, realMd5)
	}
	return err
}
